...
GoogleSignin.configure({
  webClientId: "502151032676-ckaabbahu0fsvpjun4lbdmtkf2trmut8.apps.googleusercontent.com",
  offlineAccess: true,
});

const GoogleLogin = () => {
  const [loading, setLoading] = useState(false);
  const [loggedInUser, setLoggedInUser] = useState(null as AuthUser | null);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // console.log("Backend Response:", Object.keys(loginResponse));
      ...
      useUserStore.getState().setAuthUser(loginResponse.user);
      await GoogleSignin.signOut();
      goToProfile(router, useUserStore);
